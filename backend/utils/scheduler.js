...
export const computeQueue = (jobs, algorithm, agingEnabled) => {
  ...
  const withAging = (job) => {
    ...
    return Math.max(0, Number(job.job_size) - waitMins * 3.0);
  };
  const emergencyFirst = (a, b) => {
    if (a.emergency_approved && !b.emergency_approved) return -1;
    if (!a.emergency_approved && b.emergency_approved) return 1;
    return 0;
  };
  switch (algorithm) {
    case 'fcfs':
      return [...waiting].sort((a, b) => emergencyFirst(a, b) || new Date(a.arrival_time) - new Date(b.arrival_time));
    case 'sjf':
      return [...waiting].sort((a, b) => emergencyFirst(a, b) || withAging(a) - withAging(b));
    case 'rr':
      return [...waiting].sort((a, b) => emergencyFirst(a, b) || new Date(a.arrival_time) - new Date(b.arrival_time));
    case 'priority':
      return [...waiting].sort((a, b) => {
        const starsA = Number(a.priority_stars) || 0;
        const starsB = Number(b.priority_stars) || 0;
        const scoreA = (a.emergency_approved ? 1000 : 0) + (starsA * 10) + (agingEnabled ? ((now - new Date(a.arrival_time).getTime()) / 60000) * 3.0 : 0);
        const scoreB = (b.emergency_approved ? 1000 : 0) + (starsB * 10) + (agingEnabled ? ((now - new Date(b.arrival_time).getTime()) / 60000) * 3.0 : 0);
