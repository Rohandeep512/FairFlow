...
// This explicitly forces Node to read your .env file, fixing the "ghost key" bug!
dotenv.config();
...
export const getAlgorithmRecommendation = async (serviceType, jobSizes) => {
  ...
  try {
    ...
    // SkillLoop's bulletproof JSON extractor
...
export const predictCompletion = async (jobs, algorithm) => {
  try {
    ...
    // 🔥 MASSIVE OPTIMIZATION: If queue is empty, do not waste an API call!
    if (summary.length === 0) {
    ...
    // Beefed up the prompt to be extremely strict
    ...
    // 🕵️ ADDED THIS LINE: Now we can see exactly what the AI says when it fails!
